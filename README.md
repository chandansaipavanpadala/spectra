...
# Step 5: Process Variation & SNR Sensitivity Sweep
python scripts/05_noise_robustness_sweep.py

# Step 6: Multi-Trojan Benchmark Validation Suite
python scripts/06_benchmark_generalization.py

# Step 7: FPGA Hardware Resource Utilization & Overhead Analysis
...
### 8.5 Process Variation & SNR Sensitivity Sweep
<p align="center">
  <img src="screenshots/noise_robustness_sweep.png" alt="Process Variation and SNR Sensitivity Sweep" width="95%">
</p>

> **Figure 5: PVT & SNR Sensitivity Evaluation ($P = 48$ Chips per Step).**  
> - **Panel 1 (Accuracy vs. SNR):** Sweeping SNR from $30\text{ dB}$ down to $5\text{ dB}$ demonstrates the fragility of baseline single-bin sampling (He et al., red dashed), which degrades to $50.0\%$--$72.9\%$ accuracy as clock drift shifts harmonics away from discrete sampling bins. In contrast, SPECTRA's Adaptive Harmonic Band Energy (AHBE, green solid) maintains **$100.00\%$ detection accuracy and $0.00\%$ False Positive Rate** across all noise levels.  
> - **Panel 2 (Dynamic Margin vs. Clock Drift):** Sweeping oscillator drift from $0.0\%$ to $3.0\%$ proves that AHBE continuous band integration preserves an enormous separation margin ($R_{FD,T} / R_{FD,G} > 10^4$), whereas single-bin sampling collapses to near-unity ($2.15$).

---

### 8.6 Cross-Benchmark Threat Generalization
<p align="center">
  <img src="screenshots/benchmark_comparison.png" alt="Cross-Benchmark Evaluation" width="95%">
</p>

> **Figure 6: Multi-Trojan Threat Generalization Across Trust-HUB Benchmarks.**  
> - **Panel 1 (Validation Accuracy):** Evaluates detection accuracy across three distinct hardware Trojan threat profiles: (1) Synchronous 2-Bit Counter (Implicit, He et al.), (2) Trust-HUB AES-T400 (Combinational comparator trigger inducing supply droop), and (3) Trust-HUB AES-T800 (Sequential timer leaking key bits via capacitive bursts). SPECTRA achieves **$100.00\%$ accuracy across all three threat profiles**.  
> - **Panel 2 (Dynamic Separation Margin):** Confirms autonomous Spectral Energy Analysis (SEA) cluster identification ($\int |V_T|^2 > \int |V_G|^2$) and robust dynamic separation ($1.02 \times 10^3$ for Counter, $2.13 \times 10^3$ for AES-T400, $7.46 \times 10^2$ for AES-T800) under 6% inter-die PVT variation.
